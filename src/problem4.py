...
def problem4(number_of_stairs, step_size, starting_point, window):
    startdot = rg.Circle(starting_point, 3)
    startdot.fill_color = 'black'
    startdot.attach_to(window)
    enddot = rg.Circle(rg.Point(starting_point.x + number_of_stairs*step_size, starting_point.y - number_of_stairs* step_size),3)
    enddot.fill_color = 'black'
    enddot.attach_to(window)
    for k in range(number_of_stairs):
        s_p = rg.Point(starting_point.x + k * step_size, starting_point.y - k * step_size)
        m_p = rg.Point(starting_point.x + k * step_size, starting_point.y - (k + 1) * step_size)
        e_p = rg.Point(starting_point.x + (k + 1) * step_size, starting_point.y - (k + 1) * step_size)
        line1 = rg.Line(s_p,m_p)
        line1.color = "magenta"
        line1.thickness = 3
        line1.attach_to(window)
        line2 = rg.Line(m_p, e_p)
        line2.color = 'black'
        line2.thickness = 3
